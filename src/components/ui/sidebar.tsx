...
>(
  (
    ...
    useEffect(() => {
      ...
      // Utilizamos document para agregar el event listener
    ...
    return (
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            className={cn(
              "group/sidebar-wrapper flex has-data-[variant=inset]:bg-sidebar w-full min-h-svh sidebar-styles",
              className
            )}
            ref={ref}
            style={wrapperStyle}
...
>(({ className, ...props }, ref) => {
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "shadow-none focus-visible:ring-2 focus-visible:ring-sidebar-ring w-full h-8",
        className
      )}
      {...props}
    />
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex items-center px-2 rounded-md outline-hidden ring-sidebar-ring focus-visible:ring-2 h-8 [&>svg]:size-4 font-medium text-sidebar-foreground/70 text-xs transition-[margin,opa] duration-200 ease-linear shrink-0 [&>svg]:shrink-0",
